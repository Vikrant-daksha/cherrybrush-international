...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const finalPrice = productPrice !== undefined ? Number(productPrice) : Number(price);
    ...
    // Validation
    if (!finalName || !finalDescription || !finalCollection || finalImages.length === 0 || isNaN(finalPrice)) {
      return NextResponse.json(
        {
          error: "All required fields must be provided (name, description, price, collection, and at least one image URL)",
        },
        { status: 400 }
      );
    }
